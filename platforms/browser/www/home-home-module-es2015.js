...
let HomePage = class HomePage {
    constructor(resourceListService, modalCtrl, zone, toastController, router, modalController, actionSheetController, firebaseAnalytics, _translate, translateService) {
...
let ResourceListService = class ResourceListService {
    ...
    getCategoryESList() {
        return this.db.list('categories_es', ref => ref.orderByChild('category_es')).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(changes => {
            // console.log(changes);
            return changes.map(c => (Object.assign({ key: c.payload.key }, c.payload.val())));
        }));
    }
    getCategoryFRList() {
        return this.db.list('categories_fr', ref => ref.orderByChild('category_fr')).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(changes => {
            // console.log(changes);
            return changes.map(c => (Object.assign({ key: c.payload.key }, c.payload.val())));
        }));
    }
    getCategoryVIList() {
        return this.db.list('categories_vi', ref => ref.orderByChild('category_vi')).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(changes => {
            // console.log(changes);
            return changes.map(c => (Object.assign({ key: c.payload.key }, c.payload.val())));
        }));
    }
    getCategoryZHList() {
        return this.db.list('categories_zh', ref => ref.orderByChild('category_zh')).snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(changes => {
            // console.log(changes);
            return changes.map(c => (Object.assign({ key: c.payload.key }, c.payload.val())));
        }));
    }
